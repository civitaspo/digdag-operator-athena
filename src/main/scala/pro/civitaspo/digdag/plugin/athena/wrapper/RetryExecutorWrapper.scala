...
case class ParamInWrapper(timeoutDurationMillis: Int,
                          totalWaitMillisCounter: Iterator[Int] = Stream.from(0).iterator,
                          hasOnRetry: Boolean = false)
case class ParamInRetry(e: Exception,
                        retryCount: Int,
                        retryLimit: Int,
                        retryWaitMillis: Int,
                        totalWaitMillis: Long)
case class ParamInGiveup(firstException: Exception,
                         lastException: Exception)

class RetryableException(message: String = "",
                         cause: Throwable = null)
    extends RuntimeException(message, cause)
class NotRetryableException(message: String = "",
                            cause: Throwable = null)
    extends RuntimeException(message, cause)

class RetryExecutorWrapper(exe: RetryExecutor,
                           param: ParamInWrapper)
{
    ...
    def retryIf(retryable: Exception => Boolean): RetryExecutorWrapper =
    {
        ...
        {
            override def test(t: Exception): Boolean =
            {
                retryable(t)
            }
        }
    ...
    def onRetry(f: ParamInRetry => Unit = _ => ()): RetryExecutorWrapper =
    {
        ...
        {
            override def onRetry(exception: Exception,
                                 retryCount: Int,
                                 retryLimit: Int,
                                 retryWait: Int): Unit =
            {
    ...
    def onGiveup(f: ParamInGiveup => Unit): RetryExecutorWrapper =
    {
        ...
        {
            override def onGiveup(firstException: Exception,
                                  lastException: Exception): Unit =
            {
    ...
    {
        val wrapped = if (!param.hasOnRetry) onRetry()
        else this
        try f(wrapped)
        catch {
...
object RetryExecutorWrapper
{
    ...
    def apply(exe: RetryExecutor,
              param: ParamInWrapper): RetryExecutorWrapper =
    {
        new RetryExecutorWrapper(exe, param)
    }

    def apply(): RetryExecutorWrapper =
    {
        RetryExecutorWrapper(RetryExecutor.retryExecutor(), ParamInWrapper(Int.MaxValue))
    }
